...
HelpUri: https://pwshspectreconsole.com/reference/live/invoke-spectrelive/
Locale: en-US
...
Starts live rendering for a given renderable.
The script block is able to update the renderable in real-time and Spectre Console redraws every time the scriptblock calls `$Context.refresh()`.
...
Invoke-SpectreLive -Data $layout -ScriptBlock {
    ...
    while ($true) {
        ...
        if ($lastKeyPressed.Key -eq "C" -and $lastKeyPressed.Modifiers -eq "Control") {
            # Exit the loop.
You have to treat ctrl-c as input to avoid the console readkey blocking the sigint
            return
        } elseif ($lastKeyPressed.Key -eq "Enter") {
