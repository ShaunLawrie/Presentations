...
function Write-Codeblock {
    ...
    param (
        ...
        [ValidateSet("Github", "Matrix", "Presentation")]
        [string] $Theme = "Github"
    )
...
function Write-Code {
    <#
        .SYNOPSIS
            Writes code inline to the host.
            Intended for internal use only when you want to show a code block with some nicer formatting, this won't work with long strings overlapping lines.

        .DESCRIPTION
            The Write-Code function outputs a code string to the host console with optional syntax highlighting.
            The function also supports theme selection.

        .NOTES
            Author: Shaun Lawrie
            This is a hack for my presentation.
    #>
    param (
        # The text containing the code to write to the host
        [Parameter(ValueFromPipeline=$true, Mandatory=$true)]
    ...
    if($Text -contains "`n") {
        Write-Error "You cannot use Write-Code with text containing newlines, try Write-Codeblock instead." -ErrorAction "Stop"
    }
    ...
    try {
        ...
        # Render the background
        $xPosition = $Host.UI.RawUI.CursorPosition.X
        $lineBackground = $foregroundColorEscapeCode + $gutterText + $backgroundColorEscapeCode + (" " * $codeWidth) + $resetEscapeCode
        [Console]::Write($lineBackground)
        $terminalLine = $Host.UI.RawUI.CursorPosition.Y
        
        # Render the tokens that are on this line
        ($lineTokens | Where-Object { $_.Name -eq $functionLineNumber }).Group | ForEach-Object {
            if($null -ne $_) {
                Write-Token -Token $_ -TerminalLine $terminalLine -BackgroundRgb $BackgroundRgb -GutterSize $xPosition -Highlight:$lineHighlight -Theme $Theme -SyntaxHighlight:$lineSyntax
            }
        }
    } catch {
        throw $_
